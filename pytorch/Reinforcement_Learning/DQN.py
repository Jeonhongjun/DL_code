...
def get_screen():
    ...
    return resize(screen).unsqueeze(0).to(device) #단일 샘플에 대해 unsqueeze 를 통해 가짜 임시 배치 차원을 추가, 즉 새로운 차원을 추가

env.reset() #각 time-step마다의 state
plt.figure()
...
#Epsilon greedy 정책에 따라 행동을 결정하는 유틸리티
