...
parser.add_argument('--batch-size', type=int, default=128, metavar = 'N', help='input batch size')
...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        ...
        eps = torch.randn_like(std) # torch.randn_like() : 사이즈를 튜플로 입력하지 않고 기존의 텐서로 정의
...
if __name__ == "__main__":
    for epoch in range(1, args.epochs + 1):
        ...
        test(epoch)
        with torch.no_grad():
            sample = torch.randn(64, 20).to(device) # torch.randn() : 평균이 0이고 표준편차가 1인 가우시안 정규분포를 (64, 20) shape 로 생성
