...
class Lang:
    def __init__(self, name):
        ...
        self.n_words = 2    # SOS 와 EOS 단어 숫자
...
def readLangs(lang1, lang2, reverse=False):
    ...
    # 파일을 읽고 줄로 분리
    lines = open(
        'DL_code/pytorch/text/data/%s-%s.txt' % (lang1, lang2),
        encoding='utf-8').\
        read().strip().split('\n')
    ...
    # 타언어 -> 영어 일 경우 쌍을 뒤집고, Lang 인스턴스 생성
    if reverse:
...
def train(
    ...
    input_length = input_tensor.size(0)  # instance 개수
    target_length = target_tensor.size(0)

    encoder_outputs = torch.zeros(
        max_length, encoder.hidden_size, device=device)  # 초기 Output 값 지정
    ...
    use_teach_forcing = True if random() < teach_forcing_ratio else False
...
def trainIters(
        encoder, decoder, n_iters, print_every=1000,
